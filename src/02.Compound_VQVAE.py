...
from modules.compound_modules.trainers import VQVAETrainer
from modules.compound_modules.lr_schedulers import WarmUpWrapper 
...
def main():
    ...
    # Load config
    #############
    config_path = "../code/compound_VQVAE.yml"
    ...
    for epoch in range(1, config['Train']['epochs']+1):
        ...
        print(f"[Train ({epoch})] Loss: {train_loss['loss_accum']:.4f}, VQ loss: {train_loss['vq_loss_accum']:.4f}," +
              f" Atom loss: {train_loss['atom_loss_accum']:.4f}, Atom chiral loss: {train_loss['atom_chiral_loss_accum']:.4f}," +
              f" Edge loss: {train_loss['edge_loss_accum']:.4f}")
        
        if epoch % config['Train']['eval_epoch'] == 0:
            eval_loss, patience = trainer.eval(MGraphValLoader)
            print(f"[Eval ({epoch})] Loss: {eval_loss['loss_accum']:.4f}, VQ loss: {eval_loss['vq_loss_accum']:.4f}," +
                  f" Atom loss: {eval_loss['atom_loss_accum']:.4f}, Atom chiral loss: {eval_loss['atom_chiral_loss_accum']:.4f}," +
                  f" Edge loss: {eval_loss['edge_loss_accum']:.4f}")
            
            if patience > config['Train']['patience']:
                print('patience reached max value!')
                break
