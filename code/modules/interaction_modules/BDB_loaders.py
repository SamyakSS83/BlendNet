...
class BADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        num_node = compound_graph.x.size(0)
    ...
    def get_graph(self, idx, e_start, e_end, n_atoms, start):
        ...
        data = Data(
            x=self.compound_feature_tensor[start: start + n_atoms].to(self.device),
            edge_index=edge_indices.to(self.device),
            edge_attr=self.compound_e_features_tensor[e_start: e_end].to(self.device),
            num_nodes=n_atoms
        )
        return data
