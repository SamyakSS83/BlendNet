...
from interaction_modules.models import BlendNetT
...
class BlendNetS(nn.Module):
    def __init__(self, teacher_network, config, device):
        super(BlendNetS, self).__init__()
        ...
        self.teacher_network = BlendNetT(self.config, self.device)
        self.from_pretrained_model(self.teacher_network, teacher_network)
    ...
    def pad_to_graph(self, node_reps, batch_info):
        """Convert batched node representations to padded format for interaction models"""
        if hasattr(batch_info, 'batch'):
            # PyTorch Geometric batch object
            batch_tensor = batch_info.batch
            batch_size = batch_tensor.max().item() + 1
            
            # Calculate number of nodes per graph
            num_node_list = torch.zeros(batch_size, dtype=torch.long, device=batch_tensor.device)
            for i in range(batch_size):
                num_node_list[i] = (batch_tensor == i).sum()
        else:
            # Legacy DGL format (num_node_list tensor)
            num_node_list = batch_info
            batch_size = num_node_list.size(0)
        
        total_node = 0
        max_node = torch.max(num_node_list)
        node_representations = torch.zeros(batch_size, max_node, node_reps.size()[1], device=node_reps.device)
    ...
    def forward(self, protein_data, compound_graphs, pocket_masks, compound_masks):
        ...
        # Residue encoder
        residue_representations = self.residue_encoder(protein_data, pocket_masks)        # Compound encoder
        node_representations, graph_representations = self.compound_encoder(compound_graphs)
        node_representations = self.pad_to_graph(node_representations, compound_graphs)
        ...
        # Interacton encoder
        pairwise_vectors, pairwise_maps, final_compound_features, final_pocket_features = self.intersites_predictor(residue_representations, pocket_masks, node_representations, compound_masks, pairwise_masks)
    ...
    def get_teacher_labels(self, prot_data, comp_graphs, protein_mask, compound_mask):

        ba_labels, compound_representations_for_pairwise_maps, protein_representations_for_pairwise_maps, compound_representations, protein_representations = self.teacher_network.get_teacher_predictions(prot_data, comp_graphs, protein_mask, compound_mask)
        
        pairwise_maps = torch.matmul(compound_representations_for_pairwise_maps, protein_representations_for_pairwise_maps.transpose(1,2))

        return ba_labels, pairwise_maps, compound_representations, protein_representations
...
class InteractionSitesModule(nn.Module):
    ...
    def forward(self, protein_features, pocket_masks, compound_features, compound_masks, pairwise_masks):
        ...
        ### Get pairwise vectors
        pairwise_vectors = torch.sum(final_vectors.view(final_vectors.size()[0], final_vectors.size()[1] * final_vectors.size()[2], -1), dim = 1) / torch.sum(pairwise_masks.view(-1, pairwise_masks.size()[1] * pairwise_masks.size()[2]), dim = 1).unsqueeze(1)

        return pairwise_vectors, pairwise_maps, final_compound_features, final_pocket_features
