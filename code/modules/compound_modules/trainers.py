...
from .lossess import NTXentMultiplePositives, NTXent

def move_to_device(element, device):
    '''
    takes arbitrarily nested list and moves everything in it to device if it is a torch tensor or PyTorch Geometric object
    ...
    if isinstance(element, list):
        ...
        return element.to(device) if isinstance(element,(torch.Tensor, Data, Batch)) else element
...
class PreTrainCompound():
    ...
    def train(self, MGraphDataset, MGraphIDX):
        ...
        for MGraphStep, (batch1, batch2) in enumerate(zip(tqdm(MGraphLoader1), MGraphLoader2)):
            ...
            MGraphResults["loss_cl"] += float(loss_cl.cpu().item())            # Restore original graph data
            original_graph.x[graph1_masked_atom_indices] = graph1_masked_node_labels
            original_graph.edge_attr[graph1_masked_edge_indices] = graph1_masked_edge_labels
            original_graph.edge_attr[graph1_masked_edge_indices + 1] = graph1_masked_edge_labels
            ...
            with torch.no_grad(): 
                atom_ids = self.tokenizer.get_code_indices(original_graph.x, original_graph_atom_rep)
            ...
            if self.config['MGraphTrain']['mask_edge']:
                masked_edge_index1_src, masked_edge_index1_des = graph1.edge_index[0][graph1_masked_edge_indices], graph1.edge_index[1][graph1_masked_edge_indices]
                ...
                masked_edge_index2_src, masked_edge_index2_des = graph2.edge_index[0][graph2_masked_edge_indices], graph2.edge_index[1][graph2_masked_edge_indices]
                ...
                loss_mask_edge1 = torch.tensor(0.0).to(self.device)
                loss_mask_edge2 = torch.tensor(0.0).to(self.device)
    ...
    def eval(self, MGraphDataset, MGraphIDX):
        ...
        with torch.no_grad():
            ...
            for MGraphStep, (batch1, batch2) in enumerate(zip(tqdm(MGraphLoader1), MGraphLoader2)):
                ...
                loss_cl = self.loss_cl(graph_rep1, graph_rep2)
                MGraphResults["loss_cl"] += float(loss_cl.cpu().item())                ### atom prediction ###
        ...
        if MGraphResults['total_loss'] < self.best_eval_loss:
            ...
            torch.save(self.MGraphModel.state_dict(), f"{self.config['MGraphPaths']['output_model_path']}/MGraphPretraingEncoder.pth")
    ...
    def evaluate_metrics(self, predictions, targets):
        ...
        for key, metric in self.ConformerMetric.items():
