...
class MoleculeGraphDataset(Dataset):
    ...
    def get_graph(self, e_start, e_end, n_atoms, start):
        ...
        data = Data(
            x=self.features_tensor[start: start + n_atoms].to(self.device),
            edge_index=edge_indices.to(self.device),
            edge_attr=self.e_features_tensor[e_start: e_end].to(self.device),
            num_nodes=n_atoms
        )
        return data
    ...
    def collate_fn(self, batch):
        ...
        for data in batch:
            ...
            for atom_idx in masked_atom_indices:
                batch_mask_node_labels.append(copy.deepcopy(data.x[atom_idx]).view(1, -1))

            # modify the orignal node feature of the masked node
            for atom_idx in masked_atom_indices:
                data.x[atom_idx] = torch.tensor([self.num_atom_type, 0, 0, 0, 0, 0, 0, 0, 0]) # original atom type to masked token

            if self.mask_edge:
                # create mask edge labels by copying edge features of edges that are bonded to mask atoms
                connected_edge_indices = list()

                for bond_idx, (u, v) in enumerate(zip(data.edge_index[0].cpu().numpy(), data.edge_index[1].cpu().numpy())):
                    for atom_idx in masked_atom_indices:
                ...
                if len(connected_edge_indices) > 0 :
                    ...
                    for bond_idx in connected_edge_indices[::2]: # because the
                        ...
                        batch_mask_edge_labels.append(
                            copy.deepcopy(data.edge_attr[bond_idx]).view(1, -1))
                    ...
                    accum_node += data.x.size()[0]
                    accum_edge += data.edge_attr.size()[0]
                    
                    batch_masked_atom_indices.extend(masked_atom_indices)
                    batch_masked_edge_indices.extend(connected_edge_indices)
                    
        return [Batch.from_data_list(batch), torch.tensor(batch_masked_atom_indices).to(torch.int64), torch.cat(batch_mask_node_labels, dim = 0), torch.tensor(batch_masked_edge_indices).to(torch.int64), torch.cat(batch_mask_edge_labels, dim = 0)]

def contrastive_collate(batch: List[Tuple]):
    # optionally take targets
    graphs, graphs3d, *targets = map(list, zip(*batch))
    batched_graph = Batch.from_data_list(graphs)
    batched_graph3d = Batch.from_data_list(graphs3d)
